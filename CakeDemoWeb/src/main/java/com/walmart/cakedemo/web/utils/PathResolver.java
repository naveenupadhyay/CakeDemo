...
package com.walmart.cakedemo.web.utils;
...
import com.walmart.cakedemo.configuration.Environment;
...
public class PathResolver {
    ...
    private static String getStaticPath(String path) {
        ...
        if (!req.isSecure() && Environment.PRODUCTION.getName().equals(env)) {
            ...
            sb.append(req.getScheme() + "://i" + (Math.abs(path.hashCode()) % noOfCdn + 1) + ".sdlcdn.com/static/cakedemo/");
            sb.append(path);
            return sb.toString();
        } else if (!req.isSecure() && Environment.STAGING.getName().equals(env)) {
            StringBuilder sb = new StringBuilder();
            sb.append(req.getScheme() + "://static.walmart.com:8081/static/cakedemo/");
            sb.append(path);
            return sb.toString();
        } else if (!req.isSecure() && Environment.RELEASE.getName().equals(env)) {
            StringBuilder sb = new StringBuilder();
            sb.append(req.getScheme() + "://sitestatic.release.walmart.com:8081/static/cakedemo/");
            sb.append(path);
            return sb.toString();
        } else {
            if (!req.isSecure()) {
