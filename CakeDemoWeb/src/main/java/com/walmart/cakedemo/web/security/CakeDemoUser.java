...
public class CakeDemoUser implements UserDetails, CredentialsContainer {
    ...
    private Map<String, CakeDemoAuthority> roleToAuthorities  = new HashMap<String, CakeDemoAuthority>();
    private List<GrantedAuthority>            authorities        = new ArrayList<GrantedAuthority>();

    public CakeDemoUser(User user, List<Role> childRoles) {
        this.user = user;
        if (childRoles != null && childRoles.size() > 0) {
            for (Role r : childRoles) {
                CakeDemoAuthority authority = new CakeDemoAuthority(r);
